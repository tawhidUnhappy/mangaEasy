...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate narration audio via F5-TTS.")
    ...
    parser.add_argument("--f5-tts-root", type=Path, default=None,
                        help="Path to f5-tts directory. Auto-detected if omitted.")
...
def main() -> int:
    ...
    tool_dir = (
        args.f5_tts_root.resolve()
        if args.f5_tts_root
        else resolve_tool_dir("f5-tts")
    )
    if tool_dir is None:
        print("[FATAL] Could not locate f5-tts directory.")
        return 1

    script = Path(__file__).resolve().parent.parent / "audio" / "tts_pipeline_f5tts.py"
    env = tool_env()
    env["F5_TTS_ROOT"] = str(tool_dir)
